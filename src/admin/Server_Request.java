...
public class Server_Request {
			...
			public void Upload_file(String bucket, String key_name, InputStream  file_path, ObjectMetadata meta) {
				try {
					Create_BucketS3();
					//	s3.putObject(bucket, key_name,new File(file_path));
						s3.putObject( new PutObjectRequest( bucket, key_name, file_path,meta ).withCannedAcl(CannedAccessControlList.PublicRead));
					}catch (Exception  e) {
							System.out.println(e );
			}
		}
